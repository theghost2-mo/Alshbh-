...
hotelForm.addEventListener('submit', async function(e) {
    ...
    const amenities = Array.from(document.querySelectorAll('#amenitiesTags .amenity-tag'))
        .map(tag => tag.childNodes[0].textContent.trim());

    let image = "";
    const images = [];

    // معالجة الصور (نفترض وجود صورة واحدة رئيسية فقط مبسطة الآن)
    const fileInput = document.getElementById("hotelImages");
    if (fileInput.files.length > 0) {
        const file = fileInput.files[0];
        const storageRef = ref(storage, 'hotel-images/' + Date.now() + '_' + file.name);
    ...
    alert("تم حفظ الفندق في قاعدة البيانات.");
    hotelFormModal.style.display = 'none';
    loadHotelsFromFirebase();
});
