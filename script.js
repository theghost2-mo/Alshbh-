...
const hotelForm1 = document.getElementById("addHotelForm");
...
async function loadHotels() {
    ...
    querySnapshot.forEach((doc) => {
        const data = doc.data();
        hotelList.innerHTML += `<li>${data.name} - ${data.location}</li>`;
    });
}

// تحميل الفنادق تلقائيًا عند الدخول
loadHotels();
...
const hotelForm2 = document.getElementById('hotelForm');
...
db.collection("test").add({ name: "موقع شغال" })
    .then(() => console.log("تم الاتصال بقاعدة البيانات"))
    .catch((error) => console.error("فشل الاتصال:", error));
document.getElementById("js-status").textContent = "تم تحميل السكربت وتفعيله بنجاح";
