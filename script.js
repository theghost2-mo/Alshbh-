...
import { auth } from "./firebase-config.js"; // أو من الكود الموجود في index.html إذا كتبته هناك

document.getElementById("adminLoginForm").addEventListener("submit", function (e) {
    e.preventDefault();

    const email = "admin@mahjooz.com"; // نفس اللي سجلته في Firebase
    const password = document.getElementById("adminPassword").value;
    ...
    signInWithEmailAndPassword(auth, email, password)
        ...
        .catch((error) => {
            alert("كلمة المرور غير صحيحة أو هناك خطأ!");
            console.error(error);
        });
});
...
document.getElementById("hotelForm").addEventListener("submit", async function (e) {
    ...
    try {
        ...
        console.error("حدث خطأ أثناء إضافة الفندق:", error);
        alert("فشل في إضافة الفندق، حاول مرة أخرى.");
    }
});
...
async function loadHotels() {
  const hotelsTable = document.getElementById("hotelsList");
  hotelsTable.innerHTML = ""; // نفرغ الجدول أول
  ...
  querySnapshot.forEach((doc) => {
    ...
    const row = document.createElement("tr");
    row.innerHTML = `
      <td><img src="mo.png" style="width: 50px; height: 40px;"></td>
      <td>${hotel.name}</td>
      <td>${hotel.city}</td>
      <td>${hotel.rating} نجوم</td>
      <td>${hotel.price} ريال</td>
      <td>${hotel.status === "active" ? "نشط" : "غير نشط"}</td>
      <td><button class="btn-small">تعديل</button> <button class="btn-small btn-danger">حذف</button></td>
    `;
    hotelsTable.appendChild(row);
  });
}

// نادينا الدالة بعد تحميل الصفحة
...
document.getElementById("bookingForm").addEventListener("submit", async function (e) {
    ...
    const bookingData = {
        hotelName: document.getElementById("bookingHotel").value,
        checkin: document.getElementById("bookingCheckin").value,
        checkout: document.getElementById("bookingCheckout").value,
        adults: Number(document.getElementById("bookingAdults").value),
        children: Number(document.getElementById("bookingChildren").value),
        name: document.getElementById("bookingName").value,
        email: document.getElementById("bookingEmail").value,
        phone: document.getElementById("bookingPhone").value,
        notes: document.getElementById("bookingNotes").value,
        status: "pending", // الحالة مبدئيًا قيد الانتظار
        createdAt: serverTimestamp()
    };

    try {
        await addDoc(collection(db, "bookings"), bookingData);
        alert("تم استلام الحجز بنجاح!");
...
import { collection, getDocs } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
  ...
  bookingsTable.innerHTML = ""; // تنظيف الجدول أولًا
  ...
  querySnapshot.forEach((doc) => {
    ...
    row.innerHTML = `
      ...
      <td><button class="btn-small">تفاصيل</button></td>
    `;
...
window.showBookingDetails = function (booking) {
  document.getElementById("bookingNumber").textContent = booking.id;
  document.getElementById("detailHotel").textContent = booking.hotelName;
  document.getElementById("detailUser").textContent = booking.name;
  ...
  document.getElementById("detailGuests").textContent = `بالغين: ${booking.adults}, أطفال: ${booking.children}`;
  document.getElementById("detailAmount").textContent = "يُحسب لاحقًا";
  document.getElementById("detailStatus").textContent = translateStatus(booking.status);
  document.getElementById("detailDate").textContent = booking.createdAt?.toDate ? booking.createdAt.toDate().toLocaleString() : "غير متوفر";
...
row.innerHTML = `
  ...
  <td>${translateStatus(booking.status)}</td>
  <td>
    <button class="btn-small" onclick='showBookingDetails(${JSON.stringify({
      id: doc.id,
      ...booking
    })})'>تفاصيل</button>
  </td>
...
async function updateBookingStatus(id, newStatus) {
  try {
    ...
    alert("تم تحديث حالة الحجز إلى: " + translateStatus(newStatus));
    document.getElementById("bookingDetailsModal").style.display = "none";
    loadBookings(); // نعيد تحميل الجدول
  } catch (error) {
...
async function updateBookingStatus(id, newStatus) {
  try {
    ...
    alert("تم تحديث حالة الحجز إلى: " + translateStatus(newStatus));
    document.getElementById("bookingDetailsModal").style.display = "none";
    loadBookings(); // إعادة تحميل قائمة الحجوزات بعد التحديث
  } catch (error) {
    console.error("خطأ في تحديث الحجز:", error);
    alert("حدث خطأ أثناء التحديث.");
  }
...
<button class="btn-small" onclick='showBookingDetails(${JSON.stringify({ id: doc.id, ...booking })})'>تفاصيل</button>
...
document.getElementById("bookingForm").addEventListener("submit", async function (e) {
  ...
  const pricePerNight = 300; // عدله حسب سعر الفندق الفعلي
  ...
  try {
    await addDoc(collection(db, "bookings"), bookingData);
    alert("تم تأكيد الحجز! رقم الحجز: " + bookingData.bookingNumber);
    document.getElementById("bookingForm").reset();
    document.getElementById("bookingModal").style.display = "none";
  } catch (error) {
    console.error("خطأ أثناء الحجز:", error);
    alert("فشل في الحجز، حاول مرة أخرى.");
  }
});
...
async function loadBookings() {
  ...
  querySnapshot.forEach((doc) => {
    ...
    const row = document.createElement("tr");
    row.innerHTML = `
      <td>${booking.bookingNumber || doc.id}</td>
      <td>${booking.hotelName}</td>
      <td>${booking.name}</td>
      <td>${booking.checkin} - ${booking.checkout}</td>
      <td>${booking.amount || "غير محسوب"} ريال</td>
      <td>${translateStatus(booking.status)}</td>
      <td>
        <button class="btn-small" onclick='showBookingDetails(${JSON.stringify({ id: doc.id, ...booking })})'>تفاصيل</button>
      </td>
...
function loadBookingsLive() {
  const bookingsTable = document.getElementById("allBookings");
  bookingsTable.innerHTML = "";

  onSnapshot(collection(db, "bookings"), (snapshot) => {
    bookingsTable.innerHTML = ""; // نفضي الجدول ونعيد بناءه
    ...
    snapshot.docChanges().forEach(change => {
      ...
      const row = document.createElement("tr");
      row.innerHTML = `
        <td>${booking.bookingNumber || docId}</td>
        <td>${booking.hotelName}</td>
        <td>${booking.name}</td>
        <td>${booking.checkin} - ${booking.checkout}</td>
        <td>${booking.amount || "غير محسوب"} ريال</td>
        <td>${translateStatus(booking.status)}</td>
        <td>
          <button class="btn-small" onclick='showBookingDetails(${JSON.stringify({ id: docId, ...booking })})'>تفاصيل</button>
        </td>
      `;

      // لو الحجز جديد (تمت إضافته للتو)
      if (change.type === "added") {
        row.style.backgroundColor = "#e6f7ff"; // لون خفيف يميز الجديد
        showToast(`تمت إضافة حجز جديد من ${booking.name}`);
      }
